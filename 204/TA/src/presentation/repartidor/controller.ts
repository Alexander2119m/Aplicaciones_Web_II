...
import { CreateRepartidorDto, UpdateRepartidorDto } from '../../domain';
...
export class RepartidoresController {
  ...
  public updateRepartidor = async (req: Request, res: Response) => {
    ...
    // Asegúrate de que el ID también esté presente en el objeto de actualización
    const updateDataWithId = {
      id_repartidor,
      ...updateRepartidorDto!.values,
    };
  
    // Utiliza updateMany con la misma cláusula where que tenías inicialmente
    const updatedRepartidor = await prisma.repartidor.updateMany({
      where: { id_repartidor },
      data: updateDataWithId,
    });
